...
struct NbitFastScanCodeBlock {
    ...
    void store(size_t idx, const NbitCodeStorage<D>& code) {
        for (size_t b = 0; b < BIT_WIDTH; ++b) {
            ...
            std::memcpy(plane_binary.signs, code.planes[b], BinaryCodeStorage<D>::NUM_WORDS * sizeof(uint64_t));
...
struct NbitFastScanNeighborBlock {
    ...
    NbitFastScanNeighborBlock() : count(0) { std::memset(neighbor_ids, 0xFF, sizeof(neighbor_ids)); }

    void set_neighbor(size_t slot, uint32_t id, const NbitCodeStorage<D>& code, const VertexAuxData& aux) {
        neighbor_ids[slot] = id;
        code_blocks[slot / 32].store(slot % 32, code);
        centered_norm[slot] = aux.centered_norm;
        code_ip[slot] = aux.code_ip;
        code_parent_ip[slot] = aux.code_parent_ip;
...
inline void compute_inner_products(const uint8_t lut[][16], const FastScanCodeBlock<D>& block, uint32_t* __restrict__ out) {
    ...
    __m256i acc_lo = _mm256_setzero_si256(), acc_hi = _mm256_setzero_si256(), tmp_acc = _mm256_setzero_si256();
...
inline void convert_nbit_to_distances_with_bounds(const RaBitQQuery<D>& query, const uint32_t* nbit_fastscan_sums, const uint32_t* msb_fastscan_sums, const float* centered_norm_arr, const float* code_ip_arr, const float* code_parent_ip_arr, const uint16_t* msb_popcounts, const uint16_t* weighted_popcounts, size_t count, float* __restrict__ out_dist, float* __restrict__ out_lower, float dist_qp_sq) {
    ...
    const float A_nbit = query.coeff_fastscan * inv_K, B_nbit = query.coeff_popcount * inv_K;
    const float C = query.coeff_constant, dot_slack = query.dot_slack, sqrt_dqp = std::sqrt(dist_qp_sq);
    size_t i = 0;

#ifdef __AVX512F__
    const __m512 vA_nbit_w = _mm512_set1_ps(A_nbit), vB_nbit_w = _mm512_set1_ps(B_nbit);
    const __m512 vA_msb_w = _mm512_set1_ps(query.coeff_fastscan), vB_msb_w = _mm512_set1_ps(query.coeff_popcount);
    const __m512 vC_w = _mm512_set1_ps(C), vdot_slack_w = _mm512_set1_ps(dot_slack);
    const __m512 vsqrt_dqp_w = _mm512_set1_ps(sqrt_dqp), vdist_qp_sq_w = _mm512_set1_ps(dist_qp_sq);
    const __m512 vone_w = _mm512_set1_ps(1.0f), vneg_one_w = _mm512_set1_ps(-1.0f);
    ...
    for (; i + 16 <= count; i += 16) {
        ...
        __m512 code_ip = _mm512_load_ps(code_ip_arr + i), code_parent_ip = _mm512_load_ps(code_parent_ip_arr + i);
        ...
        __m512 two_cn = _mm512_add_ps(centered_norm, centered_norm);
        __m512 cn_sq_dqp = _mm512_fmadd_ps(centered_norm, centered_norm, vdist_qp_sq_w);
        _mm512_store_ps(out_dist + i, _mm512_fnmadd_ps(two_cn, ip_est_nbit, cn_sq_dqp));
        ...
        _mm512_store_ps(out_lower + i, _mm512_fnmadd_ps(_mm512_mul_ps(two_cn, vsqrt_dqp_w), cos_upper, cn_sq_dqp));
    }
#endif

    const __m256 vA_nbit = _mm256_set1_ps(A_nbit), vB_nbit = _mm256_set1_ps(B_nbit);
    const __m256 vA_msb = _mm256_set1_ps(query.coeff_fastscan), vB_msb = _mm256_set1_ps(query.coeff_popcount);
    const __m256 vC = _mm256_set1_ps(C), vdot_slack = _mm256_set1_ps(dot_slack);
    const __m256 vsqrt_dqp = _mm256_set1_ps(sqrt_dqp), vdist_qp_sq = _mm256_set1_ps(dist_qp_sq);
    const __m256 vone = _mm256_set1_ps(1.0f), vneg_one = _mm256_set1_ps(-1.0f);
    ...
    for (; i + 8 <= count; i += 8) {
        ...
        __m256 code_ip = _mm256_load_ps(code_ip_arr + i), code_parent_ip = _mm256_load_ps(code_parent_ip_arr + i);
        ...
        __m256 two_cn = _mm256_add_ps(centered_norm, centered_norm);
        __m256 cn_sq_dqp = _mm256_fmadd_ps(centered_norm, centered_norm, vdist_qp_sq);
        _mm256_store_ps(out_dist + i, _mm256_fnmadd_ps(two_cn, ip_est_nbit, cn_sq_dqp));
        ...
        _mm256_store_ps(out_lower + i, _mm256_fnmadd_ps(_mm256_mul_ps(two_cn, vsqrt_dqp), cos_upper, cn_sq_dqp));
    }

    for (; i < count; ++i) {
        float rcp_ci = 1.0f / code_ip_arr[i], cn = centered_norm_arr[i];
        float cn_sq_d = cn * cn + dist_qp_sq;
        float ip_nbit = (A_nbit * static_cast<float>(nbit_fastscan_sums[i]) + B_nbit * static_cast<float>(weighted_popcounts[i]) + C - code_parent_ip_arr[i]) * rcp_ci;
        out_dist[i] = cn_sq_d - 2.0f * cn * ip_nbit;
        float ip_msb = (query.coeff_fastscan * static_cast<float>(msb_fastscan_sums[i]) + query.coeff_popcount * static_cast<float>(msb_popcounts[i]) + C - code_parent_ip_arr[i]) * rcp_ci;
        float cos_up = std::clamp((ip_msb + dot_slack) / sqrt_dqp, -1.0f, 1.0f);
        out_lower[i] = cn_sq_d - 2.0f * cn * sqrt_dqp * cos_up;
    }
}
...
inline void convert_msb_to_lower_bounds(const RaBitQQuery<D>& query, const uint32_t* msb_fastscan_sums, const float* centered_norm_arr, const float* code_ip_arr, const float* code_parent_ip_arr, const uint16_t* msb_popcounts, size_t count, float* __restrict__ out_lower, float dist_qp_sq) {
    ...
    const float A = query.coeff_fastscan * INV_K_PARTIAL, B = query.coeff_popcount * INV_K_PARTIAL;
    const float C = query.coeff_constant, dot_slack = query.dot_slack, sqrt_dqp = std::sqrt(dist_qp_sq);
    size_t i = 0;

#ifdef __AVX512F__
    const __m512 vA_w = _mm512_set1_ps(A), vB_w = _mm512_set1_ps(B), vC_w = _mm512_set1_ps(C);
    const __m512 vdot_slack_w = _mm512_set1_ps(dot_slack), vsqrt_dqp_w = _mm512_set1_ps(sqrt_dqp), vdist_qp_sq_w = _mm512_set1_ps(dist_qp_sq);
    const __m512 vone_w = _mm512_set1_ps(1.0f), vneg_one_w = _mm512_set1_ps(-1.0f);
    const __m512 vrcp_sqrt_dqp_w = rcp_nr(vsqrt_dqp_w);
    ...
    for (; i + 16 <= count; i += 16) {
        ...
        __m512 two_cn = _mm512_add_ps(centered_norm, centered_norm);
        _mm512_store_ps(out_lower + i, _mm512_fnmadd_ps(_mm512_mul_ps(two_cn, vsqrt_dqp_w), cos_upper, _mm512_fmadd_ps(centered_norm, centered_norm, vdist_qp_sq_w)));
    }
#endif

    const __m256 vA = _mm256_set1_ps(A), vB = _mm256_set1_ps(B), vC = _mm256_set1_ps(C);
    const __m256 vdot_slack = _mm256_set1_ps(dot_slack), vsqrt_dqp = _mm256_set1_ps(sqrt_dqp), vdist_qp_sq = _mm256_set1_ps(dist_qp_sq);
    const __m256 vone = _mm256_set1_ps(1.0f), vneg_one = _mm256_set1_ps(-1.0f);
    const __m256 vrcp_sqrt_dqp = rcp_nr(vsqrt_dqp);
    ...
    for (; i + 8 <= count; i += 8) {
        ...
        __m256 two_cn = _mm256_add_ps(centered_norm, centered_norm);
        _mm256_store_ps(out_lower + i, _mm256_fnmadd_ps(_mm256_mul_ps(two_cn, vsqrt_dqp), cos_upper, _mm256_fmadd_ps(centered_norm, centered_norm, vdist_qp_sq)));
    }
    ...
    for (; i < count; ++i) {
        ...
        float cn = centered_norm_arr[i];
        out_lower[i] = cn * cn + dist_qp_sq - 2.0f * cn * sqrt_dqp * cos_upper;
    }
}
