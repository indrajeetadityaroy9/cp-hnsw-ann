...
void prune_and_write(RaBitQGraph<D>& graph, const EncType& encoder, NodeId node, std::vector<SearchResult>& candidates) {
    ...
    std::sort(candidates.begin(), candidates.end(), [](const auto& a, const auto& b) { return a.id < b.id || (a.id == b.id && a.distance < b.distance); });
    candidates.erase(std::unique(candidates.begin(), candidates.end(), [](const auto& a, const auto& b) { return a.id == b.id; }), candidates.end());
...
void vamana_build(RaBitQGraph<D>& graph, const EncType& encoder) {
    ...
    for (;;) {
        ...
        for (NodeId uid : perm) {
            auto candidates = rabitq_search::search_graph_ceiling<D>(graph.get_vector(uid), graph, GRAPH_DEGREE, visited, medoid);

            const auto& nb = graph.get_neighbors(uid);
            for (size_t j = 0; j < nb.size(); ++j) {
                NodeId nid = nb.neighbor_ids[j];
                if (nid != INVALID_NODE) candidates.push_back({nid, graph.distance_between(uid, nid)});
            }
            std::erase_if(candidates, [uid](const SearchResult& r) { return r.id == uid; });

            uint32_t old_first = (nb.size() > 0) ? nb.neighbor_ids[0] : INVALID_NODE;
            ...
            for (size_t j = 0; j < updated_nb.size(); ++j) {
                ...
                for (size_t k = 0; k < vnb.size(); ++k) {
                    ...
                    if (w != INVALID_NODE) v_cands.push_back({w, graph.distance_between(v, w)});
                }
