...
struct VisitationTable {
    explicit VisitationTable(size_t capacity)
        : capacity_(capacity)
        , data_(std::make_unique<uint64_t[]>(capacity * 2))
        , current_epoch_(0) {}
    ...
    bool check_and_mark_estimated(NodeId node_id, uint64_t query_id) const {
        if (data_[node_id] == query_id) return true;
        data_[node_id] = query_id;
        return false;
    }

    void mark_visited(NodeId node_id, uint64_t query_id) const {
        data_[capacity_ + node_id] = query_id;
    }

    bool is_visited(NodeId node_id, uint64_t query_id) const {
        return data_[capacity_ + node_id] == query_id;
    }

    void prefetch_estimated(NodeId node_id) const {
        prefetch_t<0>(reinterpret_cast<const char*>(&data_[node_id]));
    }

    void resize(size_t new_capacity) {
        data_ = std::make_unique<uint64_t[]>(new_capacity * 2);
...
std::vector<SearchResult> search(const RaBitQQuery<D>& query, const float* raw_query, const RaBitQGraph<D>& graph, size_t k, VisitationTable& visited, NodeId entry, const GPDCalibration& cal) {
    ...
    while (!beam.empty()) {
        ...
        float worst_before = (nn.size() >= k) ? nn.worst_distance()
        ...
        if (nn.size() >= k) {
            ...
            if (exact_dist < worst_before) {
                ...
                float step_cost = std::exp(-R_B);
                exhaustion_accumulator += step_cost;

                if (exhaustion_accumulator >= static_cast<float>(GRAPH_DEGREE)) {
        ...
        estimator::estimate_neighbors<D>(query, nb, dist_qp_sq, nn.worst_distance(), !warmup, estimates);
...
std::vector<SearchResult> search_graph_ceiling(const float* raw_query, const RaBitQGraph<D>& graph, size_t k, VisitationTable& visited, NodeId entry) {
    ...
    float entry_dist = exact_l2(entry);
    ...
    while (!beam.empty()) {
        ...
        for (size_t i = 0; i < nb.size(); ++i) {
            NodeId neighbor_id = nb.neighbor_ids[i];
            if (visited.check_and_mark_estimated(neighbor_id, query_id)) continue;
            float d = exact_l2(neighbor_id);
            beam.push({d, neighbor_id});
        }
    }
